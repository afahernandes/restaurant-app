...
public class Detail_Reservasi extends ListActivity {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.detail_reservasi);
		arrayList = new ArrayList<HashMap<String, String>>();
		ip=jParser.getIP();

		SharedPreferences sharedPref = PreferenceManager.getDefaultSharedPreferences(Detail_Reservasi.this);
		...
		Intent i = getIntent();
		id_restaurant = i.getStringExtra("pk");
		...
		txtjam_datang.setOnClickListener(new View.OnClickListener() {
			@Override
		...
		public void onItemClick(AdapterView<?> parent, View view,int position, long id) {
			String kd = ((TextView) view.findViewById(R.id.kode_k)).getText().toString();
		...
		Button  btnselesai=(Button)findViewById(R.id.btnproses);
		btnselesai.setOnClickListener(new View.OnClickListener() {
			public void onClick(View arg0) {
				...
				if (ljumlah.length() < 1) {
				 ...
				 jum = Integer.parseInt(ljumlah);
				 kt = Integer.parseInt(kuota);
					}

				if(ljam_datang.length()<1){lengkapi("Jam Datang");}
	...
	private void showTimeDialog() {
		...
		timePickerDialog = new TimePickerDialog(this, new TimePickerDialog.OnTimeSetListener() {
			...
			public void onTimeSet(TimePicker view, int hourOfDay, int minute) {
				...
				int h = rightNow.get(Calendar.HOUR_OF_DAY); // return the hour in 24 hrs format (ranging from 0-23)
				int m = rightNow.get(Calendar.MINUTE); // return the hour in 24 hrs format (ranging from 0-23)
				String time1 = h+":"+m+":00";
				...
				Log.v("divv::::",String.valueOf(difference));


				if((difference/1000/60)< 60){
					gagal("Order must one hour before booking");
				}else {
	...
	class get extends AsyncTask<String, String, String> {
		...
		protected void onPreExecute() {
			super.onPreExecute();
			pDialog = new ProgressDialog(Detail_Reservasi.this);
		...
		protected String doInBackground(String... params) {
			...
			try {
				...
				params1.add(new BasicNameValuePair("id_restaurant", id_restaurant));
...
//					startActivity(i);
		...
		protected void onPostExecute(String file_url) {
			...
			runOnUiThread(new Runnable() {
				public void run() {
					ListAdapter adapter = new SimpleAdapter(Detail_Reservasi.this, arrayList,R.layout.desain_list, new String[] { TAG_id_detail,TAG_atas, TAG_bawah,},new int[] { R.id.kode_k, R.id.txtNamalkp ,R.id.txtDeskripsilkp});
	...
	class saveBk extends AsyncTask<String, String, String> {
		...
		protected void onPostExecute(String file_url) {pDialog.dismiss();
			if (sukses == 1) {
				final SharedPreferences sharedPref = PreferenceManager.getDefaultSharedPreferences(Detail_Reservasi.this);
